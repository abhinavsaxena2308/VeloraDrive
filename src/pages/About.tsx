    ...
    <div className="container mx-auto px-6 md:px-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center mb-24">
        <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="space-y-6">
          <p className="text-white/50 text-xl font-light leading-relaxed italic font-heading">
          ...
          <div className="grid grid-cols-2 gap-10 pt-6 border-t border-white/[0.06]">
      ...
      {/* Values */}
      <div className="section-padding border-t border-white/[0.04]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {values.map((v, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="space-y-4 group">
              <div className="w-10 h-1 bg-white/10 group-hover:bg-cta transition-colors duration-500" />
              <h3 className="text-2xl font-heading font-bold italic tracking-widest">{v.title}</h3>
