      ...
      <div className="p-6 flex flex-col flex-1">
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-white/30 text-xs ml-1">({car.reviews})</span>
            </div>
          </div>
          <div className="text-right">
            <span className="text-xl font-bold text-white">₹{car.price.toLocaleString()}</span>
            <span className="block text-[9px] uppercase tracking-widest text-white/30 font-semibold">/ day</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            ...
            <div key={i} className="text-center py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.04] group-hover:border-cta/10 transition-colors">
              <spec.icon size={14} className="mx-auto mb-1.5 text-cta" />
              <span className="text-[9px] text-white/40 uppercase font-semibold tracking-wider">{spec.label}</span>
            </div>
          ))}
        </div>
